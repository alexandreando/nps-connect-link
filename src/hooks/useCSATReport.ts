...
const EMPTY_STATS: CSATReportStats = { avgCsat: null, totalEvaluations: 0, positivePercent: null, negativePercent: null, csatByDay: [], scoreDistribution: [] };
...
export function useCSATReport(filters: CSATReportFilters) {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (effectiveAttendantIds) statsQuery = statsQuery.in("attendant_id", effectiveAttendantIds);
    if (filters.scores.length > 0) statsQuery = statsQuery.in("csat_score", filters.scores);
    if (tagRoomIds) statsQuery = statsQuery.in("id", tagRoomIds.slice(0, 500));
