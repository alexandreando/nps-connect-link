...
import { Download, Search, ChevronLeft, ChevronRight, Eye, CalendarIcon, Star, Loader2, RotateCcw, MoreHorizontal, Archive, CheckCircle2, X } from "lucide-react";
...
const AdminChatHistory = () => {
  ...
  const getDateRange = () => {
    if (dateFrom || dateTo) return { from: dateFrom?.toISOString(), to: dateTo ? new Date(dateTo.getTime() + 86400000).toISOString() : undefined };
    const now = new Date();
    switch (period) {
      case "today": return { from: new Date(now.getFullYear(), now.getMonth(), now.getDate()).toISOString(), to: undefined };
      case "week": { const w = new Date(now); w.setDate(w.getDate() - 7); return { from: w.toISOString(), to: undefined }; }
      case "month": { const m = new Date(now); m.setDate(m.getDate() - 30); return { from: m.toISOString(), to: undefined }; }
  ...
  const activeFilterCount = resolutionStatuses.length + attendantIds.length + csatFilters.length + tagIds.length + teamIds.length + (dateFrom ? 1 : 0) + (dateTo ? 1 : 0) + (companyId ? 1 : 0) + (companyContactId ? 1 : 0) + (period !== "all" ? 1 : 0);

  const clearAllFilters = () => {
    setResolutionStatuses([]); setAttendantIds([]); setCsatFilters([]); setTagIds([]); setTeamIds([]);
    setDateFrom(undefined); setDateTo(undefined); setSearch(""); setCompanyId(null); setCompanyContactId(null); setPeriod("all");
    handleFilterChange();
  };
